...
//                    points[i][j] = (Lawson.Evaluate(PointUV.Create(vParameters[i][j], u), p, q, circleAngle, inverseOffset, true));
//                }
//            }

//            double iteration = 2;
//            bool swap;
//            double vForce = 0.0005;
//            double angleForce = 1;
//            double iAngleForce = 0;// 0.01;
...
//                        if (((i == 17 && j == 37) || (i == 17 && j == 38)) && iteration % 4 == 0) {
...
//                        );
//                        double angleOffset = (Math.PI / 2 - Math.Abs(iAngle)) * iAngleForce;
...
//                //			newVParameters[i][j + 1] -= angleOffset/100; 
//                        }

//                    }
//                }

//                lastCumulativeError = Math.Sqrt(cumulativeErrorTally / (iSteps * 2 * jSteps * 2));
	...
	class CreateAnimateButtonCapsule : RibbonButtonCapsule {
		...
		//	double circleAngle = Math.PI / 2;
			...
			for (int i = 0; i <= frames; i++) {
				...
				Part part = activeWindow.Scene as Part;
				// TBD rewire below
				ICollection<DesignBody> designBodies = null;// = CreateLawsonButtonCapsule.CreateLawson(p, q, circleAngle, inverseOffset, part);
